...
function getAllLines (wirePath) {
  let path = []
  let input = wirePath.split(",")
  let x = 0
  let y = 0
  let steps = 0
  for (let i = 0; i < input.length; i++) {
    ...
    let xe = x
    let ye = y
    path.push({"xs": xs, "ys": ys, "xe": xe, "ye": ye, "steps": s})
  }
  return path
}

function findIntersect (path1, path2) {
  let lines1 = getAllLines(path1)
  let lines2 = getAllLines(path2)
  ...
  for (var i = 0; i < lines1.length; i++) {
      for (var j = 0; j < lines2.length; j++) {
        let line1 = lines1[i]
        let line2 = lines2[j]
        let inter = checkIntersection(line1.xs, line1.ys, line1.xe, line1.ye, line2.xs, line2.ys, line2.xe, line2.ye)
        if (inter && inter.point && inter.point.x != 0 && (intersect.findIndex(x => (x.x == inter.point.x && x.y == inter.point.y)) == -1)) {
          let steps1 = line1.steps + Math.abs(inter.point.x - line1.xs) + Math.abs(inter.point.y - line1.ys)
          let steps2 = line2.steps + Math.abs(inter.point.x - line2.xs) + Math.abs(inter.point.y - line2.ys)
          intersect.push({"x": inter.point.x, "y": inter.point.y, "steps": steps1+steps2})
        }
      }
  }
  return intersect
...
function findMinimalDistance (path1, path2) {
  let intersect = findIntersect(path1, path2)
  intersect = intersect.map(x => Math.abs(x.x) + Math.abs(x.y))
  return Math.min(...intersect)
}

function findMinimalSteps (path1, path2) {
  let intersect = findIntersect(path1, path2)
  intersect = intersect.map(x => x.steps)
  return Math.min(...intersect)
}
