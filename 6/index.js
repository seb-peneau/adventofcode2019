...
function generateGraph (graphDesc) {
  var graph = new Map()
  graphDesc.map(x => {
    var nodeDesc = x.split(")")
    var l = nodeDesc[0]
    var r = nodeDesc[1]
    if (!graph.get(l)) {
      graph.set(l, [])
...
function getNbOrbits (edge, graph, target) {
  var edgeToFind = edge
  var nbOrbits = 0
  var targetEdge = target ? target : "COM"
  while (edgeToFind != targetEdge) {
    graph.forEach((value, key) => {
      if (value.indexOf(edgeToFind) != -1 && edgeToFind != targetEdge) {
        edgeToFind = key
        nbOrbits = nbOrbits + 1
      }
    })
...
function getAllOrbits (graphDesc) {
  const graph = generateGraph(graphDesc)
  var nbTotal = 0
  let keys = [...graph.keys()]
  keys.forEach( x => {
    nbTotal = nbTotal + getNbOrbits(x, graph)
  })
  return nbTotal
}

function getCommonAncester (edge1, edge2, graph) {
  var listEdges = []
  var parentEdge1 = edge1
  var parentEdge2 = edge2
  while (parentEdge1 != "COM") {
    graph.forEach((value, key) => {
      if (value.indexOf(parentEdge1) != -1) {
        listEdges.push(key)
        parentEdge1 = key
      }
    })
  ...
  let find = "COM"
  while (parentEdge2 != "COM") {
    graph.forEach((value, key) => {
      if (value.indexOf(parentEdge2) != -1) {
        if (listEdges.indexOf(key) != -1) {
          find = key
          parentEdge2 = "COM"
        } else {
          parentEdge2 = key
